...
## Prerequisites
1. AgentMail Python SDK installed (use `/opt/hermes/.venv/bin/pip install agentmail` in Hermes environment)
...
4. Hermes-specific: Ensure using `/opt/hermes/.venv/bin/python` to run scripts to match the environment where AgentMail is installed
